...
function getStringLength(value) {
  if (String(value) !== value) return 0;
  return value.length;
}
...
function countVowels(str) {
  const arr = ['a', 'e', 'i', 'o', 'u', 'y'].join();
  let counter = 0;

  for (let i = 0; i < str.length; ) {
    if (arr.includes(str[i].toLowerCase())) counter += 1;
    i += 1;
  }
...
function isPalindrome(str) {
  const regExpr = /[\W_]/g;
  const lowRE = str.toLowerCase().replace(regExpr, '');
  const revStr = lowRE.split('').reverse().join('');
  return revStr === lowRE;
}
...
function encodeToRot13(str) {
  ...
  return str.replace(/[a-z]/gi, (char) => chars[chars.indexOf(char) + 13]);
}
